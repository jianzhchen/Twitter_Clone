...
class milestone1 extends Controller
{
	...
	public function additem(){
		...
		$cluster = Cassandra::cluster()->withContactPoints('192.168.1.48','192.168.1.69','192.168.1.72','192.168.1.70','192.168.1.93')->build();
    ...
    public function follow(){
		...
		if($follow==true){
		    ...
		    $result=$session->executeAsync($batch);
			$result=$result->get();
	    	$ok = new Ok();
	    	$session->close();
  ...
  //       );
    ...
    public function addmedia(Request $request){
		...
		$args = array(
			'id' => $id,
			'content'=> new Cassandra\Blob(bin2hex($file)),
			'type'=>$type
	  	);
		$options = array('arguments' => $args);
    	$result=$session->executeAsync($statement2, $options);
   ...
   //  	}
		...
		$session->close();
		return response(hex2bin($content->toBinaryString()),200)->header('Content-Type', $type);
    }
